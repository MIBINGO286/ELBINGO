...
function cartonGanador(grid, mode) {
  ...
  if (mode === 'diagonal') return [grid, transposed].some(arr => arr.map((_, i) => arr[i][i]).every(n => n === 'FREE' || drawn.has(n)));
  return false;
}

function verificarGanador(){
  contenedor.querySelectorAll('.carton').forEach(c=>{
    const cartonID = c.dataset.id;
    const grid = cartones.find(c=>c.id === Number(cartonID))?.grid;
    if (!grid) return;
    const mode = getMode();
    if(cartonGanador(grid, mode)){
      c.classList.add('ganador');
      alert(`¡Cartón #${cartonID} es ganador!`);
    }
  });
}
