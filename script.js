// ==================== CONFIG ====================
const API_URL = 'https://script.google.com/macros/s/AKfycbxPHJshfUqK0unphzPZUI1H-tlNsMb9QvFix05THLcIZiNMYZhe62lzvY9o-3EkYL3-1A/exec';
...
function checkWin(marks) {
  switch (currentMode) {
    ...
    case 'horizontal':
      for (let r = 0; r < 5; r++) {
        let ok = true;
        for (let c = 0; c < 5; c++) if (!marks[r][c]) { ok = false; break; }
        if (ok) return true;
      }
      break;
    case 'diagonal':
      let mainDiag = true, antiDiag = true;
      for (let i = 0; i < 5; i++) {
        if (!marks[i][i]) mainDiag = false;
        if (!marks[i][4 - i]) antiDiag = false;
      }
...
function resetGame() {
  ...
  cardsMarks.forEach((marks, cid) => {
    for (let r = 0; r < 5; r++) for (let c = 0; c < 5; c++) marks[r][c] = (r === 2 && c === 2);
  });
