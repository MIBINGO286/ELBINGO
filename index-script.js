  ...
  function renderBatch() {
    ...
    for (; index < limit; index++) {
      ...
      wrapper.innerHTML = `
        ...
        <div class="bingo-card" onclick='checkWinner(${index + 1}); openReservationModal(${index + 1})'>
...
function showWinnerOverlay(cardIndex, mode) {
  const cardEl = document.getElementById('card-' + cardIndex);
  cardEl.style.border = "5px solid gold";
  let overlay = document.getElementById("ganador-overlay");
  if (!overlay) {
    overlay = document.createElement("div");
    overlay.id = "ganador-overlay";
    overlay.style = "position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.8); color:white; font-size:30px; display:flex; align-items:center; justify-content:center; z-index:1000;";
    const texto = document.createElement("div");
    texto.id = "ganador-texto";
    overlay.appendChild(texto);
    document.body.appendChild(overlay);
  }
  document.getElementById("ganador-texto").innerHTML = "🎉 ¡CARTÓN #" + cardIndex + " GANADOR en modo " + mode.toUpperCase() + "!";
...
function checkWinner(cardIndex) {
  ...
  if (!card || card.length !== 5 || card.some(col => col.length !== 5)) {
    alert("Este cartón no está bien formado.");
    return;
  }

  const rows = [0, 1, 2, 3, 4].map(i => card.map(col => col[i]));
  const flat = rows.flat().map(c => c === 'FREE' ? 'FREE' : Number(c));
  ...
  if (won) showWinnerOverlay(cardIndex, currentMode);
  else openReservationModal(cardIndex);
}
